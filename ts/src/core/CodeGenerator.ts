...
${analysis.outputs.map(output => `  ${output.name}: any;`).join('\n')}
  ...
  private optimizeBuiltInFunctions(expression: string): string {
    return expression
      // FIX: ป้องกัน Math.Math.pow - ตรวจสอบก่อนแทนที่
      ...
      // ไม่ต้องแทนที่ pow เพราะเราใช้ Math.pow แล้วใน power operator
