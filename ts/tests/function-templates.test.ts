    ...
    describe('Real-world Integration Scenarios', () => {
        ...
        it('should handle complex hotel booking scenario', async () => {
            const config = {
                formulas: [
                    {
                        id: 'season',
                        function_call: 'get_season',
                        params: ['checkin_date']
                    },
                    {
                        id: 'base_room_rate',
                        function_call: 'seasonal_pricing',
                        params: [100, 'season']  // ✅ ใช้ result จากขั้นตอนก่อน
                    },
                    {
                    ...
                    {
                        id: 'room_tax',
                        function_call: 'calculate_room_tax',
                        params: ['weekend_adjustment', 12, 'location']
                    },
                    {
                        id: 'total_cost',
            ...
            const registry = ruleFlow.getFunctionRegistry();
            const season = registry.call('get_season', ['2024-07-06']);
            console.log('Debug season:', season); // Should be 'summer'

            expect(result.base_room_rate).toBe(130.00);
            expect(result.weekend_adjustment).toBe(195.00);
            expect(result.room_tax).toBe(28.08);
            expect(result.total_cost).toBe(223.08);
        });
        ...
        it('should handle healthcare assessment workflow', async () => {
            const config = {
                ...
                {
                    id: 'bp_category', 
                    function_call: 'blood_pressure_category',
                    params: ['systolic', 'diastolic']
                },
                {
                    id: 'health_risk_score',  
                    function_call: 'health_risk_score',
                    params: ['age', 'bmi_value', 'smoker', 'exercise_hours']
                },
                {
                    id: 'diabetes_risk_score', 
                    function_call: 'health_risk_score', 
                    params: ['age', 'bmi_value', 'family_history', 'exercise_hours']
                },
                {
                    id: 'daily_calories',  
                    function_call: 'calorie_needs',
                    params: ['age', 'gender', 'weight', 'height_cm', 'activity_level']
                }
                ]
            };
            ...
            expect(result.health_risk_score).toBe(15);  
            expect(result.diabetes_risk_score).toBeGreaterThanOrEqual(10);
