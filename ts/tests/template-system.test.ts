...
describe('Template System', () => {
  ...
  describe('Healthcare Templates', () => {
    ...
    it('should assess health risk correctly', async () => {
      ...
      expect(result.risk_factors).toBe(28); // 5 + 15 + 8 + 8
      expect(result.risk_level).toBe('High Risk');
    });
  });
  ...
  describe('Financial Templates', () => {
    it('should process loan approval correctly', async () => {
      ...
      expect(result.debt_to_income_ratio).toBe(0.25);
    ...
    it('should calculate credit score correctly', async () => {
      ...
      expect(result.payment_history_score).toBe(200); // 100 + 100
      expect(result.credit_utilization_score).toBe(100);
      expect(result.credit_score).toBeCloseTo(158, 0); // Complex calculation
      expect(result.credit_rating).toBe('Poor'); // Based on final score
    });
  });
  ...
  describe('E-commerce Templates', () => {
    it('should calculate dynamic pricing correctly', async () => {
      ...
      expect(result.final_price).toBeCloseTo(133.44, 2);
    ...
    it('should calculate shipping costs correctly', async () => {
      ...
      expect(result.total_shipping_cost).toBe(182); // (25 * 3.5 * 2) + 5 + 2
